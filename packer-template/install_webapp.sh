...
sudo unzip /opt/webapp.zip -d /opt/webapp  # Unzip webapp.zip

# List contents to verify
echo "Listing files in /opt/webapp..."
sudo ls -la /opt/webapp

# Navigate to the nested webapp directory if it exists
if [ -d /opt/webapp/webapp ]; then
...
if [ ! -f package.json ]; then
    ...
    exit 1  # Exit script if package.json is missing
fi
